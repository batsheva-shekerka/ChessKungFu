...
class Board:
    ...
    def _is_move_legal(self, piece_type, from_row, from_col, to_row, to_col):
        ...
        elif piece_type == 'N': return (dr == 2 and dc == 1) or (dr == 1 and dc == 2)
        elif piece_type == 'R':
            if dr == 0 or dc == 0: return self._is_path_clear(from_row, from_col, to_row, to_col)
            return False
        elif piece_type == 'B':
            if dr == dc: return self._is_path_clear(from_row, from_col, to_row, to_col)
            return False
        elif piece_type == 'Q':
            if (dr == 0 or dc == 0) or (dr == dc): return self._is_path_clear(from_row, from_col, to_row, to_col)
            return False
        return False
